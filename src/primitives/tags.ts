...
export async function getTags(): Promise<Tag[]> {
  try {
    const data = await getClient().graphql<TagsData>(
      'Tags',
      TAGS_QUERY,
      {}
    );
    ...
    console.warn(`[tags] Warning: ${(err as Error).message}`);
    return [];
  }
}
...
export async function createTag(
  ...
  try {
    ...
    const data = await getClient().graphql<CreateTagData>(
      'CreateTag',
      CREATE_TAG_MUTATION,
      { input }
    );
...
export async function editTag(
  ...
  try {
    ...
    const data = await getClient().graphql<EditTagData>(
      'EditTag',
      EDIT_TAG_MUTATION,
      { id, input }
    );
...
export async function deleteTag(
  ...
  try {
    await getClient().graphql<DeleteTagData>(
      'DeleteTag',
      DELETE_TAG_MUTATION,
      { id }
    );
