...
import { HOLDINGS_QUERY, AGGREGATED_HOLDINGS_QUERY } from '../queries';
...
export async function getHoldings(): Promise<Holding[]> {
  try {
    const data = await getClient().graphql<HoldingsData>(
      'Holdings',
      HOLDINGS_QUERY,
      {}
    );
    ...
    console.warn(`[holdings] Warning: ${(err as Error).message}`);
    return [];
  }
}
...
export function formatHoldingsTable(holdings: Holding[]): string {
  ...
  for (const h of sorted) {
    const value = h.security.currentPrice
      ...
      : '?';
    const ret = h.metrics.totalReturn !== null
      ...
      : '';
    lines.push(`  ${h.security.symbol}: ${h.quantity} shares @ $${h.security.currentPrice?.toFixed(2) ?? '?'} = ${value}${ret}`);
  }
