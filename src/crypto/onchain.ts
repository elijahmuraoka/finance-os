...
export async function getUsdPrices(symbols: string[]): Promise<Record<string, number>> {
  ...
  const symbolsToFetch = [
    ...new Set([...symbols, "ETH", "SOL", "BTC", "USDC", "BNB", "MNT", "POL"]),
  ];
  const coinIds = symbolsToFetch.map((s) => COINGECKO_ID_MAP[s.toUpperCase()]).filter(Boolean);
 ...
 */
export async function getSolBalance(address: string): Promise<{ sol: number; usdValue: number }> {
  ...
  const sol = (result?.value ?? 0) / 10 ** SOL_DECIMALS;
...
export async function getSolTokenBalances(address: string): Promise<TokenBalance[]> {
  ...
  const tokenMetaMap: Record<string, { symbol: string; name: string }> = {};
  try {
...
const DEBANK_CACHE_DIR =
  process.env.FINANCE_OS_CACHE_DIR || join(homedir(), ".openclaw", "cache", "finance");
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (response.status === 429) {
        if (attempt === 2)
          throw new Error(
            "DeBank rate limited — try again in a few minutes (cached data used when available)",
          );
        continue;
      }
...
export async function getAllOnchainBalances(): Promise<OnchainSnapshot> {
  ...
  if (config.ethAddresses.length > 0) {
    const walletData: Array<
      { status: "fulfilled"; value: EthWalletSnapshot } | { status: "rejected"; reason: Error }
    > = [];
      ...
      try {
        ...
        walletData.push({
          ...
          value: {
            address,
            eth: ethMainnet.eth,
            ethUsd: ethMainnet.ethUsd,
            tokens: ethMainnet.tokens,
            extraChains,
