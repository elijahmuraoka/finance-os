...
export async function getNetworthHistory(timeframe?: TimeFrame): Promise<NetworthEntry[]> {
  try {
    ...
    const data = await getClient().graphql<NetworthData>("Networth", NETWORTH_QUERY, variables);
    ...
    warn("networth", (err as Error).message);
    return [];
  }
}
